              ...
              setTerminals((prev) =>
                prev.map((term) =>
                  term.id === index
                    ? {
                        ...term,
                        text: [...term.text, initialCommands[index][lineIndex]],
                      }
                    : term,
                ),
              );
        ...
        const createChaosTerminal = () => {
          ...
          const spamInterval = setInterval(() => {
            if (messageCount < 20) {
              const randomMessage =
                randomCommands[
                  Math.floor(Math.random() * randomCommands.length)
                ];
              setTerminals((prev) =>
                prev.map((term) =>
                  term.id === terminal.id
          ...
          >
            ...
            {term.id === 3 && loadingProgress > 0 && (
              <div className="loading-bar">
                Progress: [
                {Array(Math.floor(loadingProgress / 2))
                  .fill("=")
                  .join("")}
                {Array(50 - Math.floor(loadingProgress / 2))
                  .fill(" ")
                  .join("")}
                ] {loadingProgress}%
              </div>
